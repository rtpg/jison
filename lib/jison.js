...
lrGeneratorMixin.generateAMDModule = function generateAMDModule(opt){
    opt = typal.mix.call({}, this.options, opt);
    var out = 'define([], function(){ AST={ stack:[],'+
                                    'shift: function(token,sem) {this.stack.push({name:token,semantic:sem,children:[]})},'+
'reduce: function(token,number){var newToken={name:token,'+
                                'semantic:this.stack.slice(-number).map(function(x){return x.semantic;}).join(" "),children:this.stack.slice(-number)};this.stack.splice(-number);this.stack.push(newToken)}};'
...
lrGeneratorMixin.generateModule = function generateModule (opt) {
    ...
    out += (moduleName.match(/\./) ? moduleName : "var "+moduleName)+" = (function(){AST={ stack:[],"+
                                    "shift: function(token,sem) {this.stack.push({name:token,semantic:sem,children:[]})},"+
                                    "reduce: function(token,number){var newToken={name:token,"+
                                    "semantic:this.stack.slice(-number).map(function(x){return x.semantic;}).join(' '),children:this.stack.slice(-number)};this.stack.splice(-number);this.stack.push(newToken)}};"
						              +"var rsymbols_={};";
    ...
    while (true) {
        ...
        switch (action[0]) {
            ...
            case 1: // shift
                ...
                stack.push(symbol);
                AST.shift((rsymbols_[symbol]?rsymbols_[symbol]:symbol),this.lexer.yytext);
